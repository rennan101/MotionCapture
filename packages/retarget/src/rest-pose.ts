import type {
  ...
  RestPoseData,
  RigBoneBinding,
} from "./rig-bindings.js";
import { normalize3, rotateVec, quatInvert, sub3, type Quat, type Vec3 } from "./quat.js";
...
export function captureRestPose(
  snapshots: RestBoneSnapshot[],
  bindings: RigBoneBinding[],
  anchors: CanonicalAnchorMap,
  hipsBone: string,
): RestPoseData {
  const byName = new Map<string, RestBoneSnapshot>();
  for (const s of snapshots) byName.set(s.name, s);

  const anchorPos = new Map<CanonicalPointName, Vec3>();
  const anchorMissing: string[] = [];
  for (const [point, boneName] of Object.entries(anchors) as [CanonicalPointName, string][]) {
  ...
  const bones: Record<string, RestBoneData> = {};
  const missing: string[] = [...anchorMissing];

  const anchorDir = (seg: [CanonicalPointName, CanonicalPointName]): Vec3 | null => {
