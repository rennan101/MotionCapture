function makePoint(lm, fallback) {
    if (!lm) {
        return {
...
function midpoint(a, b, weightA = 0.5) {
...
export function mapMediaPipeToCanonical(landmarks, worldLandmarks, timestamp, latencyMs, providerId = "mediapipe") {
    ...
    const leftUpperLeg = leftHip;
    const leftLowerLeg = leftKnee;
    const leftFoot = makePoint(source[31] || source[27]);
    const rightUpperLeg = rightHip;
    const rightLowerLeg = rightKnee;
    ...
    const canonical = {
        timestamp,
        overallConfidence: (pelvis.confidence + chest.confidence + leftShoulder.confidence + rightShoulder.confidence) / 4,
        root,
        pelvis,
        spine,
        chest,
...
//# sourceMappingURL=mediapipe-mapper.js.map